...
QUnit.test('Fetch a single product id from nav bar', function(assert) {
    ...
    var pids = SHARED.megaNav.getProductsIds();
...
QUnit.test('Render single regular price', function(assert) {
    ...
    SHARED.megaNav.renderDynamicPrices(jsonResponse);

    assert.deepEqual($('#qunit-fixture > span > p > span.subNavBasePrice').text(), price, "Price not found where expected");
});
...
QUnit.test('Render multiple regular prices', function(assert) {
    ...
    assert.deepEqual($('#qunit-fixture > span[data-product-id="5"] > p > span.subNavBasePrice').text(), price, "Price not found where expected");
    assert.deepEqual($('#qunit-fixture > span[data-product-id="7"] > p > span.subNavBasePrice').text(), "\u00a312.99", "Price not found where expected");
});
...
QUnit.test('Render dynamic price with no promo price', function(assert) {
    ...
    var content = SHARED.megaNav.createDynamicPriceContent(info, data);

    assert.deepEqual(content, '<p>The price is <span class="subNavBasePrice">£12.34</span></p>')
});
...
QUnit.test('Render dynamic price with promo price', function(assert) {
    ...
    assert.deepEqual(content,
            '<p>Was: <span class="subNavOldPrice">£23.45</span>' +
            '&nbsp;Now: <span class="subNavPromoPrice">£19.99</span></p>');
});
...
QUnit.test('build product infos URI with one product id', function(assert) {
    ...
    var uri = SHARED.megaNav.getProductDetailsUrl([1], '777');
...
QUnit.test('build product infos URI with multiple product ids', function(assert) {
    ...
    var uri = SHARED.megaNav.getProductDetailsUrl([1, 7, 56], '666');
